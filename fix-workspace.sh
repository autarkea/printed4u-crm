...
USER_ID=$(sqlite3 "$DB_PATH" "SELECT id FROM nc_users_v2 ORDER BY created_at ASC LIMIT 1;")
if [ -z "$USER_ID" ]; then
...
sqlite3 "$DB_PATH" "INSERT OR IGNORE INTO nc_org_users (id, org_id, user_id, roles, created_at, updated_at) VALUES ('ou_$(date +%s)', '$WORKSPACE_ID', '$USER_ID', '[\"org.owner\"]', datetime('now'), datetime('now'));"
echo "✅ Пользователь добавлен в workspace"

# Добавляем в базу
sqlite3 "$DB_PATH" "INSERT OR IGNORE INTO nc_base_users_v2 (id, base_id, user_id, roles, created_at, updated_at) VALUES ('bu_$(date +%s)', '$BASE_ID', '$USER_ID', '[\"owner\"]', datetime('now'), datetime('now'));"
